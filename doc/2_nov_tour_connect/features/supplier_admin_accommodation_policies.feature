Feature: Supplier Admin manage accommodation policies
  As a Supplier Admin ISBAT manage accommodation policies
  ...
  @javascript
  Scenario: Specifying property policy
    Given "Planet Express" has the following accommodations:
      | name                  |
      | Awesome Accommodation |
    And  I am on the "policies" step of the edit admin "Awesome Accommodation" accommodation page
    When I select "1" from "accommodation_property_policy_infant_age_from"
    And  I select "3" from "infant_age_to"
    And  I select "3" from "children_age_from"
    And  I select "14" from "accommodation_property_policy_children_age_to"
    ...
    Then "Awesome Accommodation" property policy should have the following values:
      | infant_age_from   | 1                        |
      | infant_age_to     | 3                        |
      | children_age_from | 3                        |
      | children_age_to   | 14                       |
